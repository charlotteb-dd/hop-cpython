...
public class PythonSession {
	...
	/**
	 * Hop variable key to specify the full path to the default python command.
	 * Takes precedence over both the java property and the system evnironment
	 * variable
	 */
	public static final String HOP_CPYTHON_COMMAND_PROPERTY_KEY = "hop.cpython.command";

	/**
	 * Java property to specify full path to the default python command. If not set
	 * (and env var not set), we assume 'python' is in the PATH. Takes precedence
	 * over the env var.
	 */
	public static final String CPYTHON_COMMAND_PROPERTY_KEY = "cpython.command";

	/**
	 * System environment variable to specify full path to the default python
	 * command. If not set (and java prop is not set), we assume 'python' is in the
	 * PATH.
	 */
	 ...
	 */
	private static Map<String, PythonSession> pythonServers = Collections
			.synchronizedMap(new HashMap<String, PythonSession>());

	private static Map<String, String> pythonEnvCheckResults = Collections
	...
	/**
	 * @param pythonCommand command (either fully qualified path or that which is in
	 *                      the PATH). This, plus the optional ownerID is used to
	 *                      lookup and return a session/server.
	 * @param ownerID       an optional ownerID string for acquiring the session.
	 *                      This can be used to restrict the session/server to one
	 *                      (or more) clients (i.e. those with the ID "ownerID").
	 ...
	 * @param pythonCommand command (either fully qualified path or that which is in
	 *                      the PATH). This, plus the optional ownerID is used to
	 *                      lookup a session/server.
	 * @param ownerID       an optional ownerID string for identifying the session.
	 *                      This can be used to restrict the session/server to one
	 *                      (or more) clients (i.e. those with the ID "ownerID").
	 * @param requester     the object requesting the session/server
	 * @throws SessionException if the requested session/server is not available (or
	...
	/**
	 * Returns true if the user-specified python environment/server (as specified by
	 * pythonCommand (and optional ownerID) is available.
	 *
	 * @param pythonCommand command (either fully qualified path or that which is in
	 *                      the PATH). This, plus the optional ownerID is used to
	 *                      lookup a session/server.
	...
	/**
	 * Generates a script (shell or batch) by which to execute a given python
	 * script.
	 *
	 * @param pathEntries  additional PATH entries needed for python to execute
	 *                     correctly
	 * @param pyScriptName the name of the script (in wekaPython/resources/py) to
	 *                     execute
	...
	/**
	 * Executes the python environment check script via a wrapping shell/batch
	 * script.
	 *
	 * @param pathEntries additional entries for the PATH that are required in order
	 *                    for python to execute correctly
...
//    }
	...
	private void launchServerScript(String pathEntries) throws IOException {
		...
		String script = getLaunchScript(pathEntries, "pyServer.py", windows, serverSocket.getLocalPort(),
				debug ? "debug" : "");
		if (debug) {
...
//    }
	 ...
	 * @param serverID      optional ID for the server. This, plus the path to the
	 *                      python executable can be used to uniquely identify a
	 *                      given server instance
	 * @param pathEntries   optional additional entries for the PATH that are
	 *                      required for this python instance/virtual environment to
	 *                      execute correctly
	 * @param defaultServer true if this is the default server (i.e. the python
	...
	private PythonSession(String pythonCommand, String serverID, String pathEntries, boolean defaultServer,
		...
		if (!defaultServer && pathEntries != null && pathEntries.length() > 0) {
			...
			boolean hasErrors = (envCheckResults.contains("is not available")
					...
					|| envCheckResults.contains("does not meet") || envCheckResults.contains("problem occurred")
			...
			if (!hasErrors) {
				// launch server
				log.logDetailed("Python environment check passed. Launching " + pythonCommand
						+ " with additional path [" + pathEntries + "] " + (serverID != null ? " ID " + serverID : ""));
	...
	/**
	 * If the local socket could not be created, this method shuts down the server.
	 * Otherwise, a shutdown hook is added to bring the server down when the JVM
	 * exits.
	 *
	 * @throws IOException if a problem occurs
	 */
	 ...
	 *                    really just for debugging/development where the server can
	 *                    be manually started in a separate terminal
	 * @throws IOException if a problem occurs
	 */
	private void launchServer(boolean startPython) throws IOException {
		...
		if (startPython) {
			...
			ProcessBuilder processBuilder = new ProcessBuilder(pythonCommand, serverScript, "" + localPort,
					debug ? "debug" : "");
	...
	/**
	 * Initialize the default session. This needs to be called exactly once in order
	 * to run checks and launch the server. Creates a session singleton. Assumes
	 * python is in the PATH; alternatively, can specify full path to python exe via
	 * the java property or system environment variable pentaho.cpython.command.
	 *
	 * @param pythonCommand the python command
	 * @param vars          Kettle variables
	 * @param log           logging
	 * @return true if the server launched successfully
	 * @throws HopException if there was a problem - missing packages in python, or
	 *                      python could not be started for some reason
	 */
	public static synchronized boolean initSession(String pythonCommand, IVariables vars, ILogChannel log)
		...
		if (vars != null
				&& !org.apache.hop.core.util.Utils.isEmpty(vars.getVariable(HOP_CPYTHON_COMMAND_PROPERTY_KEY))) {
			...
			if (!pyExe.exists() || !pyExe.isFile()) {
				throw new HopException(
						"Python exe: " + pythonCommand + " does not seem to exist on the " + "filesystem!");
			}
	...
	/**
	 * Transfer a pandas data frame from python and convert into Kettle rows and
	 * metadata
	 *
	 * @param frameName       the name of the pandas data frame to get
	 * @param includeRowIndex true to include the pandas data frame row index as a
	 *                        field
	...
	/**
	 * Transfer Hop rows to a python pandas data frame using Arrow format if
	 * available
	 *
	 ...
	 * buffers. Note that the buffers will potentially also contain output from the
	 * execution of arbitrary scripts too. Calling this method also resets the
	 * buffers.
	 *
	...
	/**
	 * Get the type of a variable (according to pre-defined types that we can do
	 * something with) in python
	 *
	 * @param varName the name of the variable to get the type of
	 * @return whether the variable is a DataFrame, Image, String or Unknown.
	 *         DataFrames will be converted to rows; Images (png form) and Strings
	 *         are output as a single row with fields for each variable value.
	 *         Unknown types will be returned in their string form.
	 * @throws HopException if a problem occurs
	 */
	...
	public static void main(String[] args) {
		try {
			...
			Object[] rowData = { 22, 300.22, "Hello bob", false, new Date(), new Timestamp(new Date().getTime()),
					null };
