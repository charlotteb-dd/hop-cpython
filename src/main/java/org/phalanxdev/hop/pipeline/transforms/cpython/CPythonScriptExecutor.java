...
/**
 * Step that executes a python script using CPython. The step can accept 0 or
 * more incoming row sets. Row sets are sent to python as named pandas data
 * frames. Data can be sent to python in batches, as samples, row-by-row or as
 * all available rows.
 * </p>
 * Output can be one or more variables that are set in python after the user's
 * script executes. In the case of a single variable this can be a data frame,
 * in which case the columns of the frame become output fields from this step.
 * In the case of multiple variables they are retrieved in string form or as png
 * image data - the step automatically detects if a variable is an image and
 * retrieves it as png. In this mode there is one row output from the step,
 * where each outgoing field holds the string/serializable value of a single
 * variable.
 * </p>
 * The step requires python 2.7 or 3.4. It also requires the pandas, numpy,
 * matplotlib and sklearn. The python executable must be available in the user's
 * path.
 *
...
public class CPythonScriptExecutor extends BaseTransform<CPythonScriptExecutorMeta, CPythonScriptExecutorData> {
	...
	public boolean init() {
		if (super.init()) {
			try {
				if (org.apache.hop.core.util.Utils.isEmpty(meta.getScript())
						&& org.apache.hop.core.util.Utils.isEmpty(meta.getLoadScriptFile())) {
				...
				if (inputFrames != null && inputFrames.size() > 0) {
					if (meta.getTransformIOMeta().getInfoStreams().size() != inputFrames.size()) {
						throw new HopException(BaseMessages.getString(PKG,
								"CPythonScriptExecutor.Error.InputStreamToFrameNameMismatch"));
					}
				}
				...
				if (data.script == null) {
					...
					if (meta.isLoadScriptAtRuntime()) {
						...
						if (org.apache.hop.core.util.Utils.isEmpty(scriptFile)) {
							throw new HopException(BaseMessages.getString(PKG,
									"CPythonScriptExecutor.Error.NoScriptFileNameProvided"));
						}
	...
	public boolean processRow() throws HopException {
		...
		if (first) {
			...
			if (infoStreams.size() == 0) {
				...
				if (doingReservoirSampling) {
					data.reservoirSamplersSize = Integer
							.parseInt(reservoirSamplersSize.isEmpty() ? "0" : reservoirSamplersSize);
				} else {
				...
				for (int i = 0; i < inputFrames.size(); i++) {
					...
					if (current == null) {
						throw new HopException(BaseMessages.getString(PKG,
								"CPythonScriptExecutor.Error.UnableToFindSpecifiedInputStep", transformName)); // $NON-NLS-1$
					}
		...
		if (checkFeedback(getLinesRead())) {
			logBasic(BaseMessages.getString(PKG, "CPythonScriptExecutor.Message.LineNumber", getLinesRead())); //$NON-NLS-1$
		}
		...
		if (meta.getPyVarsToGet().size() == 1) {
			...
			if (!checkIfPythonVariableIsSet(session, meta.getPyVarsToGet().get(0))) {
				if (!continueOnUnsetVars) {
					throw new HopException(BaseMessages.getString(PKG,
							"CPythonScriptExecutor.Error.PythonVariableNotSet", meta.getPyVarsToGet().get(0)));
				}
			} else {
				// Are we getting a variable (image/text) or a frame?
				PythonSession.PythonVariableType type = getPythonVariableType(session, meta.getPyVarsToGet().get(0));
				Object[][] outputRows = new Object[1][];
				if (type == PythonSession.PythonVariableType.DataFrame) {
					outputRows = data.constructOutputRowsFromFrame(session, meta.getPyVarsToGet().get(0),
							meta.isIncludeRowIndex(), getLogChannel());
					includeInputInOutput(outputRows);
				} else {
					outputRows[0] = data.constructOutputRowNonFrame(session, meta.getPyVarsToGet(), continueOnUnsetVars,
							getLogChannel());
					includeInputInOutput(outputRows);
				}
			}
	...
	protected void includeInputInOutput(Object[][] outputRows) throws HopException {
		...
		int[] rowCounts = new int[meta.isDoingReservoirSampling() ? data.reservoirSamplers.size()
				: data.frameBuffers.size()];
