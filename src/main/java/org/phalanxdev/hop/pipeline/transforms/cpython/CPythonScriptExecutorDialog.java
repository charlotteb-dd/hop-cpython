...
public class CPythonScriptExecutorDialog extends BaseTransformDialog implements ITransformDialog {
	...
	// private static final int MARGIN = PropsUi.getMargin(); // Const.MARGIN;
	...
	public CPythonScriptExecutorDialog(Shell parent, IVariables variables, Object inMeta, PipelineMeta tr,
			String sname) {
	...
	public String open() {
		...
		Control wSpacer = createShell(BaseMessages.getString(PKG, "CPythonScriptExecutorDialog.Shell.Title"));
	...
	private void addConfigureTab() {
		...
		PropsUi.setLook(inputFramesLab);
		fd = new FormData();
		fd.left = new FormAttachment(0, 0);
		...
		wtvInputFrames = new TableView(variables, wcConfig, SWT.FULL_SELECTION | SWT.MULTI, colinf, 1,
				simpleModifyListener, props);
		fd = new FormData();
	...
	private void addScriptTab() {
		...
		PropsUi.setLook(wlLoadScriptFile);
		wlLoadScriptFile.setText(BaseMessages.getString(PKG, "CPythonScriptExecutorDialog.LoadScript.Label")); //$NON-NLS-1$
		wlLoadScriptFile.setLayoutData(getFirstLabelFormData());

		wbLoadScriptFile = new Button(wcScript, SWT.CHECK);
		PropsUi.setLook(wbLoadScriptFile);
		...
		PropsUi.setLook(wtvScriptLocation);
		fd = new FormData();
		...
		// Use ScriptStyledTextComp for Python syntax highlighting in non-web
		// environments
		if (EnvironmentUtils.getInstance().isWeb()) {
			wstcScriptEditor = new StyledTextComp(variables, wcScript,
					SWT.MULTI | SWT.LEFT | SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL, true, false);
		} else {
			wstcScriptEditor = new ScriptStyledTextComp(variables, wcScript,
					SWT.MULTI | SWT.LEFT | SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL, false);
			// Enable Python syntax highlighting
			((ScriptStyledTextComp) wstcScriptEditor).addLineStyleListener("python");
		}
		PropsUi.setLook(wstcScriptEditor, Props.WIDGET_STYLE_FIXED);

		wlContinueOnUnsetVars = new Label(wcScript, SWT.RIGHT);
		PropsUi.setLook(wlContinueOnUnsetVars);
		...
		PropsUi.setLook(wbContinueOnUnsetVars);
		fd = new FormData();
		...
		PropsUi.setLook(wtvPyVarsToGet);
		fd = new FormData();
	...
	private void addFieldsTab() {
		...
		wlOutputFields.setText(BaseMessages.getString(PKG, "CPythonScriptExecutorDialog.OutFields.Label")); //$NON-NLS-1$
		...
		wbVarsToFields.setText(BaseMessages.getString(PKG, "CPythonScriptExecutorDialog.VarsToFields.Button")); //$NON-NLS-1$
		wbVarsToFields
				.setToolTipText(BaseMessages.getString(PKG, "CPythonScriptExecutorDialog.VarsToFields.Button.TipText")); //$NON-NLS-1$
		fd = new FormData();
		fd.bottom = new FormAttachment(100, -margin * 2);
		fd.left = new FormAttachment(0, 0);
		...
		wbGetFields.setToolTipText(
				BaseMessages.getString(PKG, "CPythonScriptExecutorDialog.GetFrameFields.Button.TipText"));
		fd = new FormData();
		fd.bottom = new FormAttachment(100, -margin * 2);
		...
		PropsUi.setLook(wlIncludeRowIndex);
		fd = new FormData();
		fd.bottom = new FormAttachment(100, -margin * 2);
		...
		ColumnInfo[] colinf2 = new ColumnInfo[] {
				new ColumnInfo(BaseMessages.getString(PKG, "CPythonScriptExecutorDialog.OutFields.Name"),
						ColumnInfo.COLUMN_TYPE_TEXT, false),
				new ColumnInfo(BaseMessages.getString(PKG, "CPythonScriptExecutorDialog.OutFields.Type"),
						ColumnInfo.COLUMN_TYPE_CCOMBO,
						/* ValueMeta.getAllTypes() */ ValueMetaFactory.getAllValueMetaNames(), false) };

		wtvOutputFields = new TableView(variables, wcFields, SWT.FULL_SELECTION | SWT.MULTI, colinf2, 1,
				simpleModifyListener, props);
		fd = new FormData();
	...
	private void getFrameFields(CPythonScriptExecutorMeta meta) {
		try {
			...
			// 3. If they clicked YES, clear the table and populate it with the fields we
			// gathered
			if (buttonID == SWT.YES) {
	...
	private void addRowsToProcessControllers() {
		...
		PropsUi.setLook(wlRowsToProcess);
		fd = new FormData();
		fd.left = new FormAttachment(0, 0);
		fd.right = new FormAttachment(FIRST_LABEL_RIGHT_PERCENTAGE, 0);
		fd.top = new FormAttachment(lastControl, margin);
		...
		PropsUi.setLook(wcvRowsToProcess);
		...
		wcvRowsToProcess.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (wtvInputFrames.getItemCount() > 1 && wbReservoirSampling.getSelection()
						&& wcvRowsToProcess.getText().equals(BaseMessages.getString(PKG,
								"CPythonScriptExecutorDialog.NumberOfRowsToProcess.Dropdown.RowByRowEntry.Label"))) {
					ShowMessageDialog smd = new ShowMessageDialog(shell, SWT.OK | SWT.ICON_WARNING,
		...
		wcvRowsToProcess.add(BaseMessages.getString(PKG,
				"CPythonScriptExecutorDialog.NumberOfRowsToProcess.Dropdown.AllEntry.Label"));
		wcvRowsToProcess.add(BaseMessages.getString(PKG,
				"CPythonScriptExecutorDialog.NumberOfRowsToProcess.Dropdown.RowByRowEntry.Label"));
		wcvRowsToProcess.add(BaseMessages.getString(PKG,
				"CPythonScriptExecutorDialog.NumberOfRowsToProcess.Dropdown.BatchEntry.Label"));
		...
		PropsUi.setLook(wlRowsToProcessSize);
		...
		PropsUi.setLook(wtvRowsToProcessSize);
	...
	private void addReservoirSamplingControllers() {
		...
		PropsUi.setLook(wlReservoirSamplingSize);
		...
		wtvReservoirSamplingSize.setToolTipText(
				BaseMessages.getString(PKG, "CPythonScriptExecutorDialog.ReservoirSampling.Size.TipText"));
		PropsUi.setLook(wtvReservoirSamplingSize);
	...
	/**
	 * Handles the change of the dropdown and performs the enable/disable of size,
	 * reservoir sampling and reservoir sampling size
	 */
	private void handleRowsToProcessChange() {
		...
		if (wcvRowsToProcessValue.equals(BaseMessages.getString(PKG,
				"CPythonScriptExecutorDialog.NumberOfRowsToProcess.Dropdown.BatchEntry.Label"))) {
		...
		} else if (wcvRowsToProcessValue.equals(BaseMessages.getString(PKG,
				"CPythonScriptExecutorDialog.NumberOfRowsToProcess.Dropdown.AllEntry.Label"))) {
			wtvRowsToProcessSize.setEnabled(false);
		...
		wbIncludeInputAsOutput.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (wbIncludeInputAsOutput.getSelection()) {
					ShowMessageDialog smd = new ShowMessageDialog(getParent(), SWT.OK | SWT.ICON_WARNING,
							BaseMessages.getString(PKG, "CPythonScriptExecutorDialog.InputFieldAsOutput.Dialog.Title"),
							BaseMessages.getString(PKG,
									"CPythonScriptExecutorDialog.InputFieldAsOutput.Dialog.Message"),
							false);
					smd.open();
				}
				inputMeta.setChanged();
	...
	private void addPythonOptions() {
		wConfigSelection = new MetaSelectionLine<CPythonConfig>(variables, getMetadataProvider(), CPythonConfig.class,
				wgOptions, SWT.NONE, BaseMessages.getString(PKG, "CPythonScriptExecutor.ConfigSelection.Label"),
...
//		lastControl = wtvPyServerID;
	...
	protected void getData(CPythonScriptExecutorMeta meta) {
		...
		switch (mode) {
		case BATCH:
			wcvRowsToProcess.setText(BaseMessages.getString(PKG,
					"CPythonScriptExecutorDialog.NumberOfRowsToProcess.Dropdown.BatchEntry.Label"));
			break;
		case ROW_BY_ROW:
			wcvRowsToProcess.setText(BaseMessages.getString(PKG,
...
//		setItemText(wtvPyServerID, meta.getServerID());
	...
	private void setData(CPythonScriptExecutorMeta meta) {
		...
		if (selectedText.equals(BaseMessages.getString(PKG,
				"CPythonScriptExecutorDialog.NumberOfRowsToProcess.Dropdown.BatchEntry.Label"))) {
			meta.setRowsToProcess(ProcessingMode.BATCH.getCode());
		} else if (selectedText.equals(BaseMessages.getString(PKG,
				"CPythonScriptExecutorDialog.NumberOfRowsToProcess.Dropdown.RowByRowEntry.Label"))) {
	...
	protected void checkWidgets() {
		...
		if (wbLoadScriptFile.getSelection()) {
			...
			if (wstcScriptEditor instanceof StyledTextComp) {
				((StyledTextComp) wstcScriptEditor).getTextWidget()
						.setBackground(GuiResource.getInstance().getColorDemoGray());
			}
		} else {
