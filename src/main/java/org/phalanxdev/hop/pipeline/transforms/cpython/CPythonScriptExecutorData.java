...
public class CPythonScriptExecutorData extends BaseTransformData implements ITransformData {
	...
	/**
	 * number of rows to randomly generate when trying to determine single pandas
	 * data frame output
	 */
	protected static final int NUM_RANDOM_ROWS = 100;

	/**
	 * The reservoir sampling class does not have "store all rows" behavior when the
	 * sample size is -1. Instead, it is disabled entirely when sample size < 0. So
	 * to simulate this behavior we use a default size for the -1 case. If this is
	 * not sufficient, then the user will have to manually set a size that is large
	 * enough. Note that Integer.MAX_VALUE is not used because the reservoir class
	 * allocates an array list of size equal to the sample size.
	 */
	...
	/**
	 * Holds the row meta for all incoming fields that are getting copied to the
	 * output
	 */
	...
	/**
	 * The list of processed row sets during the getRow of each incoming input
	 * stream
	 */
	...
	/**
	 * A index used to reference a line for the incoming rows when we are processing
	 * row by row with reservoir sampling active.
	 */
	 ...
	 * 
	 * protected Map<String, Integer> m_outputMetaIndexLookup = new HashMap<String,
	 * Integer>();
	 */

	/**
	 * Lookup output indexes for just input fields that are being copied to the
	 * output
	 */
	protected Map<String, Integer> nonScriptOutputMetaIndexLookup = new HashMap<String, Integer>();

	/**
	 * Variables to retrieve or columns present in pandas data frame that are not
	 * defined in the output meta
	 */
	protected List<String> varsOrColsNotDefinedInOutputMeta = new ArrayList<String>();

	/**
	 * Variables or columns defined in the output meta that are not present in the
	 * variables to retrieve or columns in the pandas data frame. Script logic
	 * (based on input values) could dictate that some variables are not set or
	 * dataframe columns not generated for some reason.
	 */
	...
	/**
	 * Initialise a lookup on output indexes of any fields being copied from input
	 * to output. User may re-order output fields in the dialog for this step, so we
	 * need the lookup.
	 */
	...
	/**
	 * Constructs outgoing rows when a single pandas data frame is being retrieved
	 * from python
	 *
	 * @param session         the session to use
	 * @param frameName       the name of the frame to get
	 * @param includeRowIndex true if the frame's row index is to be an output field
	 *                        (this can be useful in some cases - e.g. using pandas
	 *                        routine to compute quantiles of columns stores the
	 *                        quantile value in the index of the resulting data
	 *                        frame)
	...
	public Object[][] constructOutputRowsFromFrame(PythonSession session, String frameName, boolean includeRowIndex,
		...
		// Use the new Arrow-enabled method which automatically falls back to CSV if
		// needed
		...
		if (log.isDetailed()) {
			...
			if (colsNotDefinedInOutputMeta.length() > 0) {
				log.logDetailed(
						BaseMessages.getString(PKG, "CPythonScriptExecutor.Message.VarsOrColsNotDefinedInOutputMeta",
...
//	}
	 ...
	 * extracted from python. In this case there is just one output row after
	 * executing the script. Each outgoing field holds the value (either a string or
	 * a serializable) of one of the requested python variables.
	 *
	 * @param session             the session to use
	 * @param varsToGet           the list of variables to extract from python
	 * @param continueOnUnsetVars true if we should not complain if a requested
	 *                            variable is not set in python after the script
	 *                            executes
	 * @param log                 the log to use
	...
	public Object[] constructOutputRowNonFrame(PythonSession session, List<String> varsToGet,
		...
		if (first) {
			...
			if (!varsOrColsNotDefinedInOutputMeta.isEmpty() && log != null) {
				...
				log.logDetailed(BaseMessages.getString(PKG,
						"CPythonScriptExecutor.Message.VarsOrColsNotDefinedInOutputMeta", b));
			}
					...
					if (varType == PythonSession.PythonVariableType.Image) {
						if (outputRowMeta.getValueMeta(outputIndex).getType() != IValueMeta.TYPE_SERIALIZABLE) {
							throw new HopException(BaseMessages.getString(PKG,
									"CPythonScriptExecutor.Error.ImageDataMustBeStoredInSerializable"));
						}
		...
		if (tmpSet.size() != outputRowMeta.size()) {
			...
			if (log != null) {
				log.logDetailed(BaseMessages.getString(PKG, "CPythonScriptExecutor.Message.OutputFieldsNotPresentOrSet",
						b.toString()));
			}
		}
	...
	/**
	 * Generate some random rows to send to python in the case where a single
	 * variable (data frame) is being extracted and we want to try and determine the
	 * types of the output fields
	 *
	 * @param inputMeta incoming row meta
	 * @param r         Random instance to use
	 * @return a list of randomly generated rows with types matching the incoming
	 *         row types.
	 * @throws HopException if a problem occurs
	 */
	protected static List<Object[]> generateRandomRows(IRowMeta inputMeta, Random r) throws HopException {
		...
		// IValueMeta numericVM = new ValueMeta( "num", IValueMeta.TYPE_NUMBER );
		// //$NON-NLS-1$
		...
		for (int i = 0; i < NUM_RANDOM_ROWS; i++) {
			...
			for (int j = 0; j < inputMeta.size(); j++) {
				...
				case IValueMeta.TYPE_BIGNUMBER:
					...
					currentRow[j] = vm.getStorageType() == IValueMeta.STORAGE_TYPE_NORMAL
				...
				case IValueMeta.TYPE_DATE:
					...
					currentRow[j] = vm.getStorageType() == IValueMeta.STORAGE_TYPE_NORMAL ? newVal
							: tempVM.convertToBinaryStringStorageType(newVal);
					break;
			...
			try {
				...
				if (cPythonScriptExecutorMeta.isLoadScriptAtRuntime()) {
					...
					if (org.apache.hop.core.util.Utils.isEmpty(fileName)) {
						throw new HopException(BaseMessages.getString(PKG,
								"CPythonScriptExecutorData.Error.ScriptFileDoesNotExist", fileName));
					}
...
//        List<String> frameNames = cPythonScriptExecutorMeta.getFrameNames();
				...
				log.logDetailed(
						BaseMessages.getString(PKG, "CPythonScriptExecutorData.Message.DeterminingOutputFormat"));
				...
				if (type == PythonSession.PythonVariableType.DataFrame) {
					PythonSession.RowMetaAndRows result =
							// Use the new Arrow-enabled method which automatically falls back to CSV if
							// needed
	 ...
	 * @param pyPathEntries (optional) additional path entries that are required in
	 *                      the PATH for this python instance/virtual environment to
	 *                      start
	 * @param vars          vars to use
	 * @param log           log to use
	 * @throws HopException if a problem occurs
	...
	public static void initPython(String pythonCommand, String serverID, String pyPathEntries, IVariables vars,
		...
		String pyCommand = pythonCommand != null && pythonCommand.length() > 0
				&& !pythonCommand.equalsIgnoreCase("default") ? pythonCommand : null;
		String pyPath = pyPathEntries != null && pyPathEntries.length() > 0
				&& !pyPathEntries.equalsIgnoreCase("default") ? pyPathEntries : null;
	 ...
	 * @param serverID      (optional) server ID for this session (this, combined
	 *                      with the python path can be used to uniquely identify a
	 ...
	 *
	 * @param requester     the requesting object
	 * @param pythonCommand the path to the python executable for the session in
	 *                      question
	 * @param serverID      (optional) server ID (this, combined with the python
	 *                      path can be used to uniquely identify a given
	 *                      server/session)
	 * @param log           the log to use
	 * @param vars          the environment variables to use
